...
public class SearchServlet extends HttpServlet {
    ...
    private boolean hasAdvancedSearchParams(HttpServletRequest request) {
        return request.getParameter("maker") != null
                ...
                || request.getParameter("fuel") != null // Bổ sung fuel
                || request.getParameter("seat") != null;  // Bổ sung seat
    }
    ...
    private SearchCriteria parseUrlParameters(String pathInfo) throws IllegalArgumentException {
        ...
        for (int i = 0; i < parts.length - 1; i += 2) {
            ...
            switch (key.toLowerCase()) {
                ...
                case "socho":
                    try {
                        criteria.setSeat(Integer.parseInt(value));
                    } catch (NumberFormatException e) {
                        throw new IllegalArgumentException("Số chỗ không hợp lệ: " + value);
                    }
                    break;
    ...
    private boolean matchesCriteria(Car car, SearchCriteria criteria) {
        ...
        if (criteria.getFuel() != null && !criteria.getFuel().isEmpty()) {
            if (car.getFuelType() == null || !car.getFuelType().equalsIgnoreCase(criteria.getFuel())) {
                return false;
            }
        }

        if (criteria.getSeat() != null) {
            if (car.getSeatCount() == null || car.getSeatCount().intValue() != criteria.getSeat().intValue()) {
                return false;
            }
        }
