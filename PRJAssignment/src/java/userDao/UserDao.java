...
public class UserDao extends GenericDAO<User, Integer> {
    ...
    public User findByCustomerEmail(String email) {
        final String STATUS_ACTIVE = "Hoạt động"; // Có thể đưa thành constant toàn cục nếu muốn
        try {
            String jpql = "SELECT u FROM User u, Customer c "
                    + "WHERE u.customerId = c.customerId "
                    + "AND c.email = :email "
                    + "AND u.status = :statusActive";
            return em.createQuery(jpql, User.class)
                    .setParameter("email", email.trim())
                    .setParameter("statusActive", STATUS_ACTIVE)
                    .getSingleResult();
        } catch (Exception e) {
            return null;
        }
